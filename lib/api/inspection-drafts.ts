...
export interface CreateDraftInput extends Partial<Omit<InspectionDraft, 'id' | 'created_at' | 'updated_at'>> {
  visit_id: string
}

export interface UpdateDraftInput extends Partial<Omit<InspectionDraft, 'id' | 'visit_id' | 'created_at' | 'updated_at'>> {}
...
/**
 * الحصول على مسودة بواسطة visit_id
 */
export async function getDraftByVisitId(visitId: string): Promise<InspectionDraft | null> {
 ...
 * إنشاء مسودة جديدة
 */
export async function createDraft(input: CreateDraftInput, userId: string, userName: string): Promise<InspectionDraft> {
  const supabase = createClient()
...
/**
 * حفظ أو تحديث مسودة (upsert)
 */
export async function upsertDraft(input: CreateDraftInput, userId: string, userName: string): Promise<InspectionDraft> {
  const supabase = createClient()
...
export interface DraftUpdatePayload {
  ...
  old: Partial<InspectionDraft>
  new: InspectionDraft
}

/**
 * الاشتراك في تحديثات مسودة معينة
 */
export function subscribeToDraft(
  visitId: string,
  onUpdate: (payload: DraftUpdatePayload) => void
): RealtimeChannel {
  const supabase = createClient()

  const channel = supabase
    .channel(`draft:${visitId}`)
    .on(
      'postgres_changes',
      {
        event: '*',
...
/**
 * إلغاء الاشتراك من قناة real-time
 */
...
// ═══════════════════════════════════════════════════════════
// 📡 Broadcast Events (للأحداث مثل "Save and Send")
// ═══════════════════════════════════════════════════════════
...
/**
 * الاشتراك في أحداث البث (Broadcast) لمسودة معينة
 */
export function subscribeToBroadcastEvents(
  visitId: string,
  onSubmitted: (event: InspectionSubmittedEvent) => void
): RealtimeChannel {
  const supabase = createClient()

  const channel = supabase
    .channel(`broadcast:${visitId}`)
    .on('broadcast', { event: 'inspection_submitted' }, (payload: any) => {
      onSubmitted(payload.payload as InspectionSubmittedEvent)
    })
    .subscribe()
...
/**
 * إرسال حدث "تم الإرسال" لجميع المستخدمين المتصلين
 */
export async function broadcastInspectionSubmitted(
  visitId: string,
  userId: string,
  userName: string
): Promise<void> {
  ...
  const channel = supabase.channel(`broadcast:${visitId}`)
